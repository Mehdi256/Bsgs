...
def bsgs_exact_key(pubkey_point, z1, z2):
    ...
    if z1G == pubkey_point:
        print('============== KEYFOUND ==============')
        print('BSGS FOUND PVK ', hex(z1))
        print('======================================')
        s = hex(z1)
        f=open(u"SAVE.BSGS.FOUND.txt","a+")
        f.write(s)
        f.write('\n')            
        f.close()
        found = True
...
def bsgs_keys(pubkey_point, k1, k2):
    ...
    if k1 < w:
        ...
        if idx is not None and idx >= 0:
            print('============== KEYFOUND ==============')
            print('BSGS FOUND PVK ', hex(idx+1))
            print('======================================')
            s3 = hex(idx+1)
            f=open(u"SAVE.BSGS.FOUND.txt","a+")
            f.write(s3)
...
#            print("Removed Found Q from Search   ")
    ...
    if len(Qlist) == 0:
        ...
        lastitem = k2
        k1 = randk(a, b)
        print('PVK Found. {0} scanned in {1:.2f} sec. New range [+] hex: {2}'.format(scan_str(seq), curr, hex(k1)))
        st = time.time()
        k2 = k1 + seq
        k1G = bytes(bytearray(scalar_multiplication(k1)))
#        print('[+] k1:', hex(k1))
